...
CzATCWallet::CzATCWallet(std::string strWalletFile)
{
    ...
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZATCSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZATCSeed_deprecated()) {
                    LogPrintf("%s: Updated zATC seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
...
bool CzATCWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZATCCount(nCountLastUsed);
    else if (!walletdb.ReadZATCCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzATCWallet::Lock()
{
    seedMaster = 0;
}

void CzATCWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzATCWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZATC(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzATCWallet::LoadMintPoolFromDB()
{
...
void CzATCWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzATCWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzATCWallet::SyncWithChain(bool fGenerateMintPool)
{
...
bool CzATCWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZATC(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
...
void CzATCWallet::SeedToZATC(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzATCWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzATCWallet::GenerateDeterministicZATC(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzATCWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzATCWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
