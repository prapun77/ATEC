...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeAtecAmount"))
        settings.setValue("nAnonymizeAtecAmount", 1000);

    nAnonymizeAtecAmount = settings.value("nAnonymizeAtecAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeAtecAmount"))
        SoftSetArg("-anonymizeatecamount", settings.value("nAnonymizeAtecAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in atec.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeAtecAmount:
            return QVariant(nAnonymizeAtecAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeAtecAmount:
            nAnonymizeAtecAmount = value.toInt();
            settings.setValue("nAnonymizeAtecAmount", nAnonymizeAtecAmount);
            emit anonymizeAtecAmountChanged(nAnonymizeAtecAmount);
            break;
        case CoinControlFeatures:
