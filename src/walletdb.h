...
// Copyright (c) 2016-2018 The ATEC developers
...
public:
    ...
    bool WriteZATCSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZATCSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZATCSeed_deprecated(uint256& seed);
    bool EraseZATCSeed();
    bool EraseZATCSeed_deprecated();

    bool WriteZATCCount(const uint32_t& nCount);
    bool ReadZATCCount(uint32_t& nCount);
