...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 ATC at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zATC send
enum ZerocoinSpendStatus {
    ZATC_SPEND_OKAY = 0,                            // No error
    ZATC_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZATC_WALLET_LOCKED = 2,                         // Wallet was locked
    ZATC_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZATC_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZATC_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZATC_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZATC_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZATC_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZATC_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZATC_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZATC_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZATC_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZATC_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZATC_SPENT_USED_ZATC = 14,                      // Coin has already been spend
    ZATC_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    ZATC_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    bool CreateZATCOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    ...
    */
    ...
    CzATCWallet* zwalletMain;

    bool fFileBacked;
    bool fWalletUnlockAnonymizeOnly;
    std::string strWalletFile;
    bool fBackupMints;
    std::unique_ptr<CzATCTracker> zpivTracker;
    ...
    void setZWallet(CzATCWallet* zwallet)
    {
        zwalletMain = zwallet;
        zpivTracker = std::unique_ptr<CzATCTracker>(new CzATCTracker(strWalletFile));
    }

    CzATCWallet* getZWallet() { return zwalletMain; }
    ...
    /** zATC reset */
    boost::signals2::signal<void()> NotifyzATCReset;
